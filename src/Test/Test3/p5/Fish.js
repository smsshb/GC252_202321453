...
class Fish extends Boid {
  ...
  createFish(location, maxSpeed, maxForce) {
    this.mousePositionOld = createVector(
  ...
  update() {
    ...
    if (
      this.mousePosition.x !== this.mousePositionOld.x ||
      this.mousePosition.y !== this.mousePositionOld.y
    ) {
  ...
  render() {
    ...
    let finLLocation = createVector(
      this.location.x + this.body.spine[3][0],
      this.location.y + this.body.spine[3][1]
    );
    let finRLocation = createVector(
      this.location.x + this.body.spine[3][0],
      this.location.y + this.body.spine[3][1]
    );
    ...
    let tailLocation = createVector(
      this.location.x + this.body.spine[this.numBodySegments - 1][0],
      this.location.y + this.body.spine[this.numBodySegments - 1][1]
    );
    fill(this.mainColor, this.transparency);
    ...
    let headLocation = createVector(
      this.location.x + this.body.spine[1][0],
      this.location.y + this.body.spine[1][1]
    );
    ...
    if (this.lastAteTimer > 0) {
      this.lastAteTimer--;
      let mouthSize =
        this.bodySizeW * 0.05 * (float(this.lastAteTimer) / (frameRate() / 2));
    ...
    if (this.getFoodEaten() > 11 && this.isExploding) {
      ...
      textSize(36);
      text(
        "😢 Oh no! You gave it too much food,\n I guess you did not have anything else to do.",
        width / 2,
        height / 2
      );
    }
  ...
  checkBorders() {
    ...
    if (this.location.x < -buffer) this.location.x = width + buffer;

    if (this.location.y < -buffer) this.location.y = height + buffer;

function Organism(stats) {
  ...
  this.vel = new p5.Vector(
    random(-this.maxVel, this.maxVel),
    random(-this.maxVel, this.maxVel)
  );
  ...
  (this.colorize = function (foodRank) {
    ...
    (this.findPred = function (organisms) {
      ...
      for (var i = 0; i < organisms.length; i++) {
        ...
        if (
          this.foodRank == this.other.foodRank - 1 &&
          this.loc.dist(this.other.loc.get()) < this.visionR + this.other.radius
        ) {
    ...
    (this.findPrey = function (organisms) {
      ...
      for (var i = 0; i < organisms.length; i++) {
        ...
        if (
          this.foodRank == this.other.foodRank + 1 &&
          place.dist(this.other.loc.get()) < this.visionR + this.other.radius
        ) {
          // if is underneath, eat it
          if (place.dist(this.other.loc.get()) <= this.radius) {
            ecosystem.killOrganism(
              this.foodRank - 1,
              i,
              this.other.calories * 0.25,
              this.other.loc.get()
            );
            this.calories += this.other.calories * 0.75;
            this.hunting = false;
            this.nearestDist = null;
            this.nearestPV = null;
